...
class Processor(Thread):
    ...
    def run(self):
        """
        Gets poses for all objects of interest that could (possibly) be in an image.
        ...
        while self.running:
            ...
            for obj in list(obj_ids.keys()):
                #data['objects']
                obj_trans = self.vicon.aquire_Object_Trans(object_name=obj)
                obj_rot = self.vicon.aquire_Object_RotQuaternion(object_name=obj)
    ...
    def writeData(self, data, titles):
        path = os.path.join(self.folder_path, self.name, "images")
        with open(
