...
class Camera(Thread):
    def __init__(self, id, size_factor=4):
        ...
        self.device = cvb.DeviceFactory.open(
            os.path.join(cvb.install_path(), "drivers", "GenICam.vin"), port=self.id
        )
    ...
    def saveImage(self, index):
        """Stores image in the local folder

        Args:
            index (Number): current image index

        Returns:
            Boolean: true if image was saved, false otherwise
        """
        try:
            ...
            np.save(
                os.path.join("images", f"camera_{self.id}", f"image_{index}.npy"), image
            )
            return True
        except:
            return False
